...
int Which_bin(float data, float bin_maxes[], int bin_count,
   ...
   /*******************************************************************/
   int down=0;//bin标号的下界
   int up=bin_count-1;//bin标号的上界
   int mid;
   float bin_max,bin_min;//用来记录某一个bin的上下界
   
   while(down<=up){//开始进行二分查找
      mid=(up+down)/2;
      bin_max=bin_maxes[mid];//当前查找的bin的上界
      if(mid==0){//特殊情况，若当前bin为0号bin
      		bin_min=min_meas;
	  }
	  else{//通常情况下
	  		bin_min=bin_maxes[mid-1];//当前查找的bin的下界
	  }
      if(data>=bin_max){
      		down=mid+1;//若数据大于当前bin[mid]的最大值，则down=mid+1
	  }
      else if(data<bin_min){
      		up=mid-1;//若数据小于当前bin[mid]的最小值，则up=mid-1;
	  }
      else{
      		return mid;//若数据在bin[mid]的上下界范围之内，则找到，返回当前bin的标号，即mid
	  }
   }
   /******************************************************************/
